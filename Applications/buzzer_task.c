...
void Buzzer_Task(void const * argument){
	...
	//初始化倍镜舵机
	fn_ocular_servo_init();
	
    while(1){
		//__HAL_TIM_SetCompare(&htim8, TIM_CHANNEL_2, temp);
		if(IF_KEY_PRESSED_B && !last_B_key && ocular_servo.mode == OCULAR_CLOSE)
		{
			ocular_servo.mode = OCULAR_OPEN;
		}
		...
		last_B_key = ctl.key.v & KEY_PRESSED_OFFSET_B;
        //进入陀螺仪校准模式或者校准完成提示音
