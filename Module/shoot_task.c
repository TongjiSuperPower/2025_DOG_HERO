...
void fn_fric_state(void){
	...
	if(IF_RC_SW1_MID && IF_RC_SW2_MID && (shoot_data.fric_state == FRIC_DOWN || shoot_data.fric_state == FRIC_OFF) && IF_KEY_PRESSED_G && shoot_data.last_key_pressed_g == 0)
	{
		shoot_data.fric_state = FRIC_WARMING;
		shoot_data.last_key_pressed_g = 1;
	}
	else if(!IF_KEY_PRESSED_G && shoot_data.last_key_pressed_g == 1)
	{
		shoot_data.last_key_pressed_g = 0;
	}
	if(IF_RC_SW1_MID && IF_RC_SW2_MID && shoot_data.fric_state == FRIC_WARMING && !IF_KEY_PRESSED_G && shoot_data.last_key_pressed_g == 0)
	{
		shoot_data.fric_state = FRIC_OFF;
	}
...
void fn_ShootMove(void){
	...
	//摩擦轮
	if (shoot_data.fric_state == FRIC_WARMING)
	{
        ...
        gimbal_motor3508_data[0].given_current = 1000;
		gimbal_motor3508_data[1].given_current = 1000;
		gimbal_motor3508_data[2].given_current = 1000;
	}
	...
	if(shoot_data.fric_state == FRIC_ON){
		...
		if(shoot_data.infact_shoot_speed != shoot_data.last_infact_shoot_speed && shoot_data.infact_shoot_speed > 15.9f && IF_MOUSE_PRESSED_LEFT){
            shoot_data.fric_speed -= 20.0f;
		}
